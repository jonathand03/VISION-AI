...
void app_main(void)
{
    // Variáveis para os dados brutos (Raw)
    int16_t ax_raw, ay_raw, az_raw;
    int16_t gx_raw, gy_raw, gz_raw;

    // Variáveis para os dados convertidos (unidades físicas)
    float ax_g, ay_g, az_g;     // Aceleração em 'g'
    float gx_dps, gy_dps, gz_dps; // Giroscópio em 'graus por segundo' (degrees per second)
    ...
    // Calibração de offset simples (opcional, mas recomendado)
    // Para um projeto real, você deve calcular o offset médio
    // com o sensor parado e subtraí-lo aqui.
    // Ex: gx_raw -= gyro_x_offset;

    while (1) {
        // 1. Ler os dados brutos do sensor
        esp_err_t ret = mpu6050_read_data(&ax_raw, &ay_raw, &az_raw, &gx_raw, &gy_raw, &gz_raw);

        if (ret == ESP_OK) {
            
            // 2. Converter os dados brutos para unidades físicas
            ...
            // Giroscópio (em 'graus por segundo')
            ...
            // 3. Imprimir os dados convertidos
            //    (%.2f imprime um float com 2 casas decimais)
            printf("--------------------------------------\n");
            printf("Aceleração (g):   X=%.2f, Y=%.2f, Z=%.2f\n", ax_g, ay_g, az_g);
            printf("Giroscópio (°/s): X=%.2f, Y=%.2f, Z=%.2f\n", gx_dps, gy_dps, gz_dps);

        } else {
            ESP_LOGE(TAG, "Falha ao ler dados do MPO6050");
        }
